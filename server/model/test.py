...
def scale_input(input_array, scaler_path, window):
    ...
    flattened = input_array.reshape((sample_count*window, -1))
...
# Convert frame count(int) to timestamp(str, MM:SS:FF)
# TODO: -> M:SS:FF
...
def convert_timestamp_csv(label_array, output_csv_path):
    ...
    # Create a DataFrame
    df = pd.DataFrame(timestamps, columns=['Modapts', 'In', 'Out', 'Duration'])
...
def inference(input_path:str, gt_path:str=None, config:Dict[str, str]={})->str:
    ...
    output_folder = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), "results")
...
def run_inference(input_video:str, gt_path:str=None, config_path:str=None)->str:
    ...
    # HOME Directory is upper directory of this file
